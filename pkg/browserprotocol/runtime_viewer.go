...
func (command RuntimeViewerCommand) Validate() error {
	if !validRuntimeAttemptIdentity(command.AttemptIdentity) ||
		!validUUID(command.BrowserSessionID) ||
		!validUUID(command.AttachmentID) ||
...
func validRuntimeAttemptIdentity(
	...
	return validUUID(identity.RunID) &&
		validUUID(identity.AttemptID) &&
		validUUID(identity.LeaseID) &&
		identity.FencingToken > 0 &&
		validUUID(identity.NodeID) &&
		validUUID(identity.AgentID) &&
		validUUID(identity.WorkerID) &&
