...
func (control *browserHumanControl) run(ctx context.Context) {
	...
	for {
		...
		case extension, ok := <-commands:
			...
			command, err := browserprotocol.DecodeRuntimeViewerCommand(
				extension.Payload,
			)
			if err != nil ||
...
func (control *browserHumanControl) handleCommand(
	...
	command browserprotocol.RuntimeViewerCommand,
) {
	...
	case browserprotocol.RuntimeViewerClaim:
		control.claim(ctx, command)
	case browserprotocol.RuntimeViewerInput:
		control.input(ctx, command)
	case browserprotocol.RuntimeViewerRelease:
		control.release(ctx, command)
	case browserprotocol.RuntimeViewerResume:
		control.resume(command)
	case browserprotocol.RuntimeViewerTerminate:
		control.terminate()
	}
}
...
func (control *browserHumanControl) startFrames(
	...
	go func() {
		...
		for {
			...
			sequence++
			framePayload := browserprotocol.RuntimeViewerFrame{
			...
			raw, err := browserprotocol.EncodeRuntimeViewerFrame(framePayload)
			if err != nil {
				return
			}
			reply, err := control.extensions.Publish(
				...
				openlinker.RuntimeExtensionRequest{
					Type:    browserprotocol.RuntimeViewerFrameMessage,
					Payload: raw,
				},
			)
			if err != nil ||
				reply == nil ||
				reply.Type != browserprotocol.RuntimeViewerFrameAckMessage {
				return
			}
			ack, err := browserprotocol.DecodeRuntimeViewerFrameAck(
				reply.Payload,
			)
			if err != nil ||
