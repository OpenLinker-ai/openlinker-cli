...
const EXTENSION_SOURCE = path.join(CLI_ROOT, "native-chrome", "extension");
...
async function validateStateRoot(rootRepository, stateRoot) {
  ...
  const cli = await realpath(CLI_ROOT);
  if (!isWithin(cli, root)) {
    throw new Error("openlinker-cli must be inside the supplied root repository");
  }
  ...
  if (!isWithin(normalizedStateRoot, stateBase) || isWithin(normalizedStateRoot, cli)) {
